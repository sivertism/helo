...
typedef struct {
	uint16_t match_id;
	uint8_t winner_id; // top bit used to indicate draw/win?
	uint8_t loser_id;
	bool draw;
} match_t;
...
char * render(int fd) {
	...
	int res = read(fd, template, count);
	if (res == 0) {
	...
	// Render match table
	char table_header [] = "<table><tr><th>Match</th><th>Winner</th><th>Loser</th><th>Draw</th></tr>\n%MATCH_TABLE%";
	char row_template [] = "<tr><td>%MID%</td><td>%W%</td><td>%L%</td><td>%D%</td></tr>\n%MATCH_TABLE%";

	rendered = str_replace(rendered, "%MATCH_TABLE%", table_header);

	if (n_matches) {
		for (int i=n_matches-1; i >= 0 && i >= n_matches - 10; i--) {
			rendered = str_replace(rendered, "%MATCH_TABLE%", row_template);

			sprintf(with, "%d", matches[i].match_id);
	...
	char table_header_ratings [] = "<table><tr><th>Player</th><th>ELO</th><th>Wins</th><th>Losses</th></tr>\n%RATINGS_TABLE%";
	char row_template_ratings [] = "<tr><td>%P%</td><td>%E%</td><td>%W%</td><td>%L%</td></tr>\n%RATINGS_TABLE%";

	rendered = str_replace(rendered, "%RATINGS_TABLE%", table_header_ratings);

	for (int i=1; i<=n_players; i++) {
		rendered = str_replace(rendered, "%RATINGS_TABLE%", row_template_ratings);
...
void print_match(const match_t * match) {
	...
	printf("Match %d:\n\tWinner: %d\n\tLoser: %d\n\tDraw: %b\n",
			match->match_id, match->winner_id & 0x7f, match->loser_id, draw);
}

int get_player_id(const char * player) {
	for (int i=0; i<sizeof(player_ids); i++) {
...
void get_player_by_id(char *res, int id) {
	if ( id == 0 ) {
		...
		printf("Finding player with id %d -> %s, len=%d\n", id, &players[player_ids[id]], len);
		memcpy(res, &players[player_ids[id]], len);
		res[len] = 0;
	}
}

void game_logic(const char * key, const char * value) {
	...
	if (current_match.winner_id && current_match.loser_id) {
		...
		current_match.match_id = n_matches;
	}
}

int post (const char * request) {
	printf("Received POST request\n");
	printf("%s\n", request);
	printf("Let's try to find the body\n");
...
void main () {
	...
	while(keep_running) {
		...
		char * rendered = render(opened_fd);
		close(opened_fd);
		if (!rendered) {
